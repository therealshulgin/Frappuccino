+++
title = "A relay, not a vault: designing an Android app for the moment it is seized"
description = "What a forensic examiner finds on a seized phone: the concrete Android attack surface of an encrypted-testimony app, built from the assumption that the device is the adversary's."
path = "writeups/relay-not-a-vault"
+++
...
Companion to [the proof-stack writeup](@/proving-ai-assisted-crypto.md), which
covers how the cryptography is verified. This one is the concrete Android attack
surface under seizure. **Not yet audited by an external party**; the repository is
public at [github.com/therealshulgin/Frappuccino](https://github.com/therealshulgin/Frappuccino).
...
[Frappuccino](@/_index.md) (a fork of Tella FOSS, Apache 2.0) takes the other
route: not a better safe, but no safe at all. The architecture (chunked
end-to-end upload to a blind relay while filming, a reading key that exists only
on paper, a device that can encrypt but never decrypt its own past) is on
[the positioning page](@/positioning.md) and proven in
[the companion writeup](@/proving-ai-assisted-crypto.md). This piece is only the
forensic surface that design produces, on the phone and on the server, and it is
honest about what leaks.
...
court-grade chain of custody, is on
[the positioning page](@/positioning.md#4-what-frappuccino-does-not-claim)):
...
The repository is public at [github.com/therealshulgin/Frappuccino](https://github.com/therealshulgin/Frappuccino). If the model is wrong
about what a seized phone gives up, the fastest way to find out is for a forensic
examiner to take one apart.
