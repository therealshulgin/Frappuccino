...
public class Preferences {
    ...
    // BT-HIGH-09 (audit Red/Blue Team V2, 2026-04-17) :
    // isSecretModeActive / setSecretModeActive supprimés — gate dead d'un
    // feature "secret mode" Tella legacy qui stockait un password en XML clair.
    ...
    // BT-HIGH-09 — getSecretPassword / setSecretPassword supprimés (dead code,
    // cleartext XML). Toute re-introduction d'un mécanisme mot-de-passe doit
    // passer par PinProtectedStore (Argon2id + XChaCha20-Poly1305).
    ...
    /**
     * Phase 6 — ratchet inactivity auto-lock delay in ms. Drives the
     * re-PIN-on-return auto-lock in {@code V2LockTimeoutController},
     * separately from the JWT-clear {@code lock_timeout}. Default 15 min;
     * {@code -1} (NEVER) disables it.
     */
    public static long getRatchetAutoLockMs() {
        return getLong(SharedPrefs.RATCHET_AUTOLOCK_MS, 900000L);
    }

    public static void setRatchetAutoLockMs(long value) {
        setLong(SharedPrefs.RATCHET_AUTOLOCK_MS, value);
    }

    /**
     * OEM-killer guidance (2026-06-23) — has the one-time "run without
     * restriction" guide already been shown on an aggressive-OEM device?
     * See {@code OemKillerHelper} / {@code StreamActivity}.
     */
    public static boolean isOemGuideShown() {
        return getBoolean(SharedPrefs.OEM_GUIDE_SHOWN, false);
    }

    public static void setOemGuideShown(boolean shown) {
        setBoolean(SharedPrefs.OEM_GUIDE_SHOWN, shown);
    }

    /**
     * OEM-killer feedback loop — timestamp (epoch ms) of the most recent
     * foreground-service kill already surfaced to the user, so the reactive
     * guide fires once per new kill (not on every resume).
     */
    public static long getLastSeenKillMs() {
        return getLong(SharedPrefs.LAST_SEEN_KILL_MS, 0L);
    }

    public static void setLastSeenKillMs(long value) {
        setLong(SharedPrefs.LAST_SEEN_KILL_MS, value);
    }
