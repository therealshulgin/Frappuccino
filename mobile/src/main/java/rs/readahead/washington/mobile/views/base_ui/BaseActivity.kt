...
abstract class BaseActivity : AppCompatActivity() {
    ...
    private fun maybeEnableSecurityScreen() {
        // BT-HIGH-08 — FLAG_SECURE inconditionnel (threat model militant).
        // La préférence Preferences.isSecurityScreenEnabled() est inaccessible
        // en onboarding, ce qui rendait la mnémonique BIP-39 screenshotable
        // par défaut. Plus de toggle : screenshots toujours désactivés.
        window.setFlags(
            WindowManager.LayoutParams.FLAG_SECURE,
            WindowManager.LayoutParams.FLAG_SECURE
        )
    ...
    open fun setThemeStyle() {
        // Frappuccino V2 uses a single AppTheme (dark OSINT), no runtime theme switching.
    }
}